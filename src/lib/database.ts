...
export class DatabaseClient {
  ...
  async saveSession(session: Session): Promise<void> {
    ...
    try {
      const existingRes = await this.safeFetch(`${this.baseUrl}/sessions?userId=${encodeURIComponent(userId)}&documentId=${encodeURIComponent(documentId)}`);
  ...
  async getSession(documentId: string, userId: string): Promise<Session | null> {
    try {
      const res = await this.safeFetch(`${this.baseUrl}/sessions?documentId=${encodeURIComponent(documentId)}&userId=${encodeURIComponent(userId)}`);
      if (res.status === 404) 
        return null;
      if (!res.ok) 
      ...
      console.error(`Error al obtener la sesión para documentId ${documentId} y userId ${userId}:`, error);
      throw error;
    }
  }
  ...
  async getDocument(id: string): Promise<Document | null> {
    try {
      ...
      if (res.status === 404) 
        return null;
      if (!res.ok) 
        throw new Error(`GET /documents/${id} ${res.status}`);
  ...
  async getDocuments(): Promise<Document[]> {
    try {
      ...
      if (!res.ok) 
        throw new Error(`GET /documents ${res.status}`);
