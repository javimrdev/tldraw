...
export class DatabaseClient {
	...
	constructor(
		baseUrl: string = process.env.JSON_SERVER_URL || "http://localhost:3001",
	) {
	...
	async saveDocument(document: Document): Promise<void> {
		...
		try {
			const existingRes = await this.safeFetch(
				`${this.documentsUrl}/${encodeURIComponent(id)}`,
			);

			if (existingRes.ok) {
				const putRes = await this.safeFetch(
					`${this.documentsUrl}/${encodeURIComponent(id)}`,
					{
						method: "PUT",
				...
				if (!putRes.ok)
					throw new Error(`PUT /documents/${id} ${putRes.status}`);
		...
		try {
			...
			if (existingRes.ok) {
				...
				if (Array.isArray(existingSessions) && existingSessions.length > 0) {
					...
					if (!putRes.ok)
						throw new Error(`PUT /sessions/${sessionId} ${putRes.status}`);
				} else {
	...
	async getSession(
		...
		try {
			...
			if (!res.ok)
				throw new Error(
					`GET /sessions?documentId=${documentId}&userId=${userId} ${res.status}`,
				);
