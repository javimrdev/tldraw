...
export const documentsRouter = createTRPCRouter({
    ...
    deleteSession: baseProcedure
        .input(z.object({ documentId: z.string(), userId: z.string() }))
        .mutation(async (opts) => {
            const res = await fetch(`${dbClient.baseUrl}/sessions?documentId=${encodeURIComponent(opts.input.documentId)}&userId=${encodeURIComponent(opts.input.userId)}`);
            if (res.ok) {
                const sessions = await res.json();
                for (const session of sessions) {
                    await fetch(`${dbClient.baseUrl}/sessions/${session.id}`, { method: 'DELETE' });
                }
            }
            return { ok: true };
        })
